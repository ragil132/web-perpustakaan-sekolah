$(function () {
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        customize: function (doc) {
                            ...
                            doc.content[1].table.widths = ['8%', '25%', '17%', '8%',
                                '8%', '17%', '17%'];
    ...
    $('body').on('click', '.btn-belumkembali', function () {
        var dataId = $(this).attr('data-id');
        Swal.fire({
            icon: 'warning',
            text: 'Apakah buku benar-benar belum dikembalikan?',
            showCancelButton: true,
            confirmButtonText: 'Ya',
            confirmButtonColor: '#58d8a3',
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: "/bukubelumdikembalikan/" + dataId,
                    type: "GET",
                    dataType: "JSON",
                    success: function (response) {
                        switch (response) {
    ...
    $('body').on('click', '.btn-hapuspeminjaman', function () {
        var dataId = $(this).attr('data-id');
        Swal.fire({
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "/hapusdatapeminjaman/" + dataId,
                    type: "GET",
                    dataType: "JSON",
                    success: function (response) {
                        switch (response) {
